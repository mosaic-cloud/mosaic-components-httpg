...
-type(amqp_method_field_name() ::
       ...
       | consumer_tag | version_major | version_minor | requeue
       | server_properties | locales | mechanisms | client_properties
       | response | mechanism | locale | challenge
       | prefetch_size | response | global | channel_max
       | frame_max | heartbeat | table | channel_max
       | requeue | frame_max | heartbeat | no_local
       | virtual_host | exchange | capabilities | insist
       | known_hosts | consumer_tag | host | string_op
       | known_hosts | consumer_tag | reply_code | reply_text
       | class_id | method_id | ticket | out_of_band
       | routing_key | mandatory | immediate | active
       | integer_result | active | reply_code | ticket
       | reply_code | reply_text | details | content_checksum
       | reply_code | reply_text | queue | class_id
       | method_id | string_result | exchange | routing_key
       | realm | exchange | exclusive | passive
       | active | write | read | delivery_tag
       | routing_key | ticket | integer_1 | exchange
       | reply_text | routing_key | ticket | exchange
       | type | arguments | passive | durable
       | exchange | auto_delete | internal | nowait
       | result | arguments | delivery_tag | routing_key
       | ticket | exchange | if_unused | nowait
       | requeue | ticket | queue | passive
       | durable | exclusive | dtx_identifier | auto_delete
       | nowait | arguments | queue | integer_2
       | message_count | consumer_tag | consumer_count | ticket
       | meta_data | queue | exchange | delivery_tag
       | routing_key | nowait | arguments | prefetch_size
       | prefetch_count | redelivered | global | ticket
       | queue | nowait | ticket | message_count
       | consumer_tag | no_local | ticket | queue
       | nowait | consumer_tag | prefetch_count | consumer_tag
       | nowait | consumer_tag | identifier | content_size
       | operation | staged_size | ticket | ticket
       | exchange | queue | routing_key | mandatory
       | immediate | identifier | no_ack | reply_code
       | reply_text | exchange | routing_key | integer_4
       | consumer_tag | delivery_tag | redelivered | exchange
       | ticket | routing_key | identifier | delivery_tag
       | multiple | queue | delivery_tag | requeue
       | consumer_tag | redelivered | integer_3 | no_ack
       | prefetch_size | prefetch_count | consume_rate | global
       | exclusive | message_count | ticket | queue
       | consumer_tag | no_local | nowait | exclusive
       | nowait | consumer_tag | consumer_tag | nowait
       | consumer_tag | result | cluster_id | ticket
       | exchange | routing_key | mandatory | immediate
       | operation | reply_code | queue | reply_text
       | exchange | routing_key | consumer_tag | delivery_tag
       | exchange | string_1 | queue | delivery_tag
       | string_2 | multiple | no_ack | nowait
       | integer_op | exclusive )).
