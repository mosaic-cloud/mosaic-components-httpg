...
-type(amqp_method_field_name() ::
       ( consumer_tag | consumer_tag | nowait | consumer_tag
       | identifier | content_size | table | staged_size
       | requeue | exchange | ticket | exchange
       | routing_key | mandatory | immediate | identifier
       | string_op | reply_code | reply_text | exchange
       | routing_key | consumer_tag | delivery_tag | redelivered
       | exchange | immediate | routing_key | identifier
       | ticket | delivery_tag | multiple | delivery_tag
       | requeue | queue | string_result | result
       | exchange | dtx_identifier | routing_key | reply_text
       | prefetch_size | prefetch_count | consume_rate | global
       | arguments | exchange | ticket | queue
       | consumer_tag | no_local | routing_key | exclusive
       | nowait | consumer_tag | consumer_tag | nowait
       | consumer_tag | ticket | exchange | routing_key
       | mandatory | consumer_tag | immediate | reply_code
       | meta_data | reply_text | exchange | delivery_tag
       | routing_key | consumer_tag | delivery_tag | exchange
       | redelivered | queue | integer_1 | prefetch_count
       | nowait | message_count | ticket | version_major
       | version_minor | server_properties | locales | mechanisms
       | queue | client_properties | response | mechanism
       | integer_2 | locale | challenge | no_ack
       | prefetch_size | response | global | channel_max
       | frame_max | heartbeat | channel_max | frame_max
       | heartbeat | ticket | no_local | virtual_host
       | capabilities | insist | known_hosts | queue
       | consumer_tag | host | known_hosts | consumer_tag
       | reply_code | reply_text | consumer_tag | class_id
       | method_id | redelivered | integer_op | ticket
       | out_of_band | integer_3 | routing_key | mandatory
       | active | integer_result | active | reply_code
       | no_ack | reply_code | reply_text | details
       | content_checksum | reply_code | reply_text | exclusive
       | class_id | method_id | message_count | exchange
       | routing_key | realm | nowait | exclusive
       | passive | active | write | read
       | delivery_tag | ticket | exchange | routing_key
       | ticket | exchange | integer_4 | type
       | result | passive | durable | cluster_id
       | auto_delete | internal | nowait | arguments
       | delivery_tag | ticket | exchange | queue
       | if_unused | nowait | operation | requeue
       | ticket | queue | passive | string_1
       | durable | exclusive | delivery_tag | auto_delete
       | nowait | arguments | string_2 | queue
       | message_count | multiple | consumer_count | no_ack
       | operation | ticket | nowait | queue
       | exchange | routing_key | nowait | exclusive
       | arguments | prefetch_size | prefetch_count | global
       | ticket | nowait | queue | nowait
       | ticket | message_count | consumer_tag | no_local
       | ticket | consumer_tag | queue | if_unused
       | requeue | if_empty )).
