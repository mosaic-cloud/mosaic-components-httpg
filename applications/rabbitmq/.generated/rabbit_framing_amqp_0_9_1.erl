...
-type(amqp_method_field_name() ::
       ...
       | prefetch_count | ticket | queue | no_ack
       | delivery_tag | consumer_tag | redelivered | exchange
       | exclusive | routing_key | message_count | arguments
       | message_count | queue | message_count | consumer_count
       | ticket | queue | exchange | delivery_tag
       | routing_key | nowait | arguments | consumer_tag
       | multiple | ticket | ticket | queue
       | nowait | nowait | version_major | version_minor
       | server_properties | locales | mechanisms | queue
       | client_properties | response | mechanism | if_unused
       | locale | challenge | response | ticket
       | if_empty | channel_max | frame_max | heartbeat
       | arguments | channel_max | frame_max | nowait
       | heartbeat | virtual_host | capabilities | insist
       | prefetch_size | known_hosts | global | reply_code
       | reply_text | class_id | method_id | ticket
       | queue | ticket | no_local | out_of_band
       | no_ack | channel_id | message_count | nowait
       | active | exchange | active | consumer_tag
       | reply_code | reply_text | class_id | routing_key
       | method_id | consumer_tag | realm | exclusive
       | mandatory | passive | active | requeue
       | write | read | immediate | ticket
       | reply_code | ticket | exchange | queue
       | type | passive | durable | auto_delete
       | internal | nowait | exchange | arguments
       | routing_key | ticket | exchange | if_unused
       | routing_key | nowait | ticket | destination
       | source | routing_key | nowait | reply_text
       | arguments | cluster_id | ticket | destination
       | source | exchange | routing_key | nowait
       | arguments | delivery_tag | requeue | routing_key
