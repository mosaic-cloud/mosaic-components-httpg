...
-type(amqp_method_field_name() ::
       ( channel_max | frame_max | heartbeat | consumer_tag
       | virtual_host | capabilities | ticket | insist
       | known_hosts | consumer_tag | host | known_hosts
       | queue | consumer_tag | reply_code | reply_text
       | class_id | method_id | no_local | ticket
       | redelivered | out_of_band | routing_key | mandatory
       | active | integer_3 | active | reply_code
       | reply_code | reply_text | details | no_ack
       | reply_code | reply_text | class_id | method_id
       | exchange | exclusive | routing_key | message_count
       | realm | exclusive | passive | active
       | nowait | write | read | delivery_tag
       | ticket | exchange | routing_key | integer_4
       | ticket | exchange | type | passive
       | durable | auto_delete | internal | nowait
       | cluster_id | arguments | delivery_tag | ticket
       | exchange | if_unused | nowait | queue
       | requeue | ticket | queue | passive
       | durable | exclusive | auto_delete | string_1
       | nowait | arguments | delivery_tag | queue
       | message_count | consumer_count | string_2 | multiple
       | ticket | queue | no_ack | exchange
       | routing_key | nowait | nowait | arguments
       | prefetch_size | exclusive | prefetch_count | global
       | ticket | queue | nowait | nowait
       | ticket | message_count | consumer_tag | no_local
       | ticket | queue | if_unused | if_empty
       | consumer_tag | nowait | consumer_tag | requeue
       | message_count | consumer_tag | nowait | ticket
       | queue | exchange | routing_key | arguments
       | identifier | content_size | staged_size | prefetch_size
       | prefetch_count | requeue | global | exchange
       | routing_key | mandatory | exchange | immediate
       | identifier | integer_1 | integer_2 | reply_code
       | string_op | reply_text | exchange | routing_key
       | result | consumer_tag | delivery_tag | redelivered
       | exchange | routing_key | identifier | immediate
       | result | delivery_tag | multiple | table
       | dtx_identifier | integer_op | delivery_tag | requeue
       | integer_result | operation | consumer_tag | string_result
       | content_checksum | prefetch_size | prefetch_count | consume_rate
       | reply_text | global | ticket | exchange
       | queue | consumer_tag | no_local | exclusive
       | nowait | routing_key | consumer_tag | consumer_tag
       | nowait | consumer_tag | ticket | exchange
       | routing_key | mandatory | immediate | meta_data
       | consumer_tag | reply_code | reply_text | exchange
       | routing_key | delivery_tag | consumer_tag | delivery_tag
       | exchange | queue | redelivered | operation
       | ticket | version_major | version_minor | server_properties
       | locales | ticket | mechanisms | client_properties
       | response | mechanism | locale | queue
       | challenge | response | channel_max | no_ack
       | frame_max | heartbeat )).
