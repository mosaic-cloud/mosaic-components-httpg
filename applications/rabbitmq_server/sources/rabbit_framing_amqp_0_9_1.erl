...
-type(amqp_method_field_name() ::
       ( redelivered | prefetch_count | exchange | ticket
       | queue | no_ack | consumer_tag | delivery_tag
       | redelivered | exclusive | exchange | routing_key
       | arguments | message_count | version_minor | consumer_tag
       | delivery_tag | ticket | nowait | arguments
       | nowait | routing_key | multiple | ticket
       | exchange | if_unused | nowait | ticket
       | destination | if_unused | source | routing_key
       | nowait | arguments | cluster_id | if_empty
       | ticket | destination | source | routing_key
       | nowait | arguments | nowait | delivery_tag
       | requeue | requeue | ticket | queue
       | passive | durable | exclusive | auto_delete
       | nowait | arguments | ticket | queue
       | message_count | message_count | consumer_count | exchange
       | ticket | queue | exchange | routing_key
       | nowait | routing_key | arguments | ticket
       | mandatory | queue | nowait | version_major
       | message_count | server_properties | locales | mechanisms
       | requeue | queue | client_properties | queue
       | response | mechanism | locale | challenge
       | exchange | response | ticket | channel_max
       | frame_max | heartbeat | arguments | routing_key
       | channel_max | frame_max | heartbeat | virtual_host
       | capabilities | insist | prefetch_size | reply_text
       | known_hosts | global | reply_code | reply_text
       | class_id | method_id | exchange | ticket
       | queue | no_local | out_of_band | no_ack
       | channel_id | routing_key | nowait | active
       | active | consumer_tag | reply_code | reply_text
       | class_id | method_id | consumer_tag | realm
       | exclusive | passive | active | write
       | read | consumer_tag | immediate | ticket
       | reply_code | ticket | exchange | delivery_tag
