...
-type(amqp_method_field_name() ::
       ( ticket | consume_rate | global | version_major
       | version_minor | server_properties | locales | identifier
       | mechanisms | client_properties | response | mechanism
       | locale | integer_1 | challenge | response
       | channel_max | integer_2 | frame_max | heartbeat
       | ticket | channel_max | frame_max | heartbeat
       | reply_code | integer_3 | virtual_host | queue
       | capabilities | insist | known_hosts | reply_text
       | host | consumer_tag | known_hosts | string_2
       | reply_code | reply_text | exchange | class_id
       | method_id | no_local | table | out_of_band
       | routing_key | string_op | active | integer_result
       | active | reply_code | reply_text | details
       | content_checksum | reply_code | reply_text | result
       | class_id | method_id | realm | consumer_tag
       | exclusive | passive | active | write
       | read | ticket | ticket | queue
       | exchange | type | string_1 | passive
       | durable | auto_delete | internal | nowait
       | arguments | exchange | ticket | exchange
       | if_unused | nowait | routing_key | ticket
       | no_ack | queue | passive | durable
       | exclusive | auto_delete | nowait | exclusive
       | arguments | queue | message_count | integer_4
       | consumer_count | nowait | result | ticket
       | queue | exchange | routing_key | nowait
       | arguments | prefetch_size | prefetch_count | global
       | operation | ticket | queue | nowait
       | ticket | message_count | consumer_tag | no_local
       | ticket | queue | if_unused | if_empty
       | nowait | consumer_tag | integer_op | message_count
       | operation | consumer_tag | nowait | ticket
       | queue | delivery_tag | exchange | routing_key
       | arguments | identifier | content_size | requeue
       | staged_size | routing_key | prefetch_size | prefetch_count
       | global | exchange | routing_key | mandatory
       | immediate | ticket | queue | no_local
       | consumer_tag | no_ack | dtx_identifier | exclusive
       | nowait | string_result | consumer_tag | delivery_tag
       | consumer_tag | redelivered | consumer_tag | nowait
       | identifier | consumer_tag | delivery_tag | multiple
       | ticket | exchange | routing_key | mandatory
       | immediate | reply_code | reply_text | exchange
       | routing_key | consumer_tag | delivery_tag | redelivered
       | exchange | routing_key | prefetch_size | prefetch_count
       | ticket | queue | no_ack | delivery_tag
       | redelivered | exchange | routing_key | message_count
       | exclusive | nowait | cluster_id | meta_data
       | consumer_tag | delivery_tag | multiple | consumer_tag
       | nowait | delivery_tag | requeue | consumer_tag
       | requeue | ticket | exchange | requeue
       | mandatory | immediate | reply_code | reply_text
